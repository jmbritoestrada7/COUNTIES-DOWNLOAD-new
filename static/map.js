(() => {
  ...
  function strRows(hit) {
    ...
    if (!rows.length) return '<div class="empty-str">No STR-by-acreage values were found in the Excel file.</div>';
  ...
  async function saveDrawings(){ if(applyingRemote)return; status.textContent='Saving drawn areas…'; const res=await fetch(`/api/projects/${projectId}/drawings`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({drawings:drawingsGeoJSON(),sender:clientId})}); status.textContent=res.ok?'Drawn areas saved and synchronized.':'Error saving drawn areas.'; }
  ...
  document.getElementById('uploadForm').addEventListener('submit',async e=>{e.preventDefault();const file=document.getElementById('excelFile').files[0];if(!file)return;const fd=new FormData();fd.append('file',file);status.textContent='Processing Excel…';const res=await fetch(`/api/projects/${projectId}/excel`,{method:'POST',body:fd});const data=await res.json();if(!res.ok){status.textContent=data.error||'Error uploading Excel';return;}counties=data.counties;refreshCountyStyles();status.textContent=`${data.count} counties loaded.`;fitVisibleCounties();});
